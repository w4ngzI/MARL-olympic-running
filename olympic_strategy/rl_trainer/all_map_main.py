...
# force = np.arange(-100, 220, 20).tolist()
# angle = np.arange(-30, 34, 4).tolist()
# actions_map, dim = {}, 0
# for a1 in force:
#     for a2 in angle:
#         actions_map.update({dim:[a1, a2]})
#         dim += 1
...
def get_args():
    ...
    parser.add_argument("--opponent", default='random', type=str, choices=['random', 'self'])
...
def main(args):
    ...
    if args.opponent == 'random':
        ...
        opponent_agent = model  # self-play based training
    ...
    for map in range(1, 12, 1):
        env.specify_a_map(map)

        cur_episode = 0

        while cur_episode < args.max_episodes:
            state = env.reset(args.shuffle_map)

            ctrl_agent_index = random.randint(0, 1) # shuffle ctrl id
            ...
            while True:
                with torch.no_grad():
                    action_opponent = opponent_agent.choose_action(
                        np.array(obs_oppo_agent).flatten(),
                        # train=False
                    )  
